document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!comprasGuardadas) {
    ...
    comprasGuardadas.forEach((element) => {
      contenidoDeCarrito += `
              <tr id="nombresTabla">
                <td><img src="${element.image}" id="imagenProductoComprado"class="img-thumbnail img-fluid">${element.name}</td>
                <td class="align-middle">${element.price}</td>
                <td class="align-middle"><input type=number min="0" id="cuantificador" value="${element.quantity}"></input></td>
