...
const productosAscendentes = data.products.sort((a, b) => a.cost - b.cost);
...
botonDescendente.addEventListener("click", () =>{
  const productosDescendentes = data.products.sort((a,b) => b.cost - a.cost)
...
botonFiltro.addEventListener("click", () =>{
  const productosFiltrados = data.products.filter((product) => product.cost >= filterInput.value && product.cost <= filterInput2.value )
...
botonRelevancia.addEventListener("click", () =>{
  const productosRelevantes = data.products.sort((a, b) => b.soldCount - a.soldCount);
